...
The system MUST allow admins to set an optional maximum number of concurrent bookings for a service. A null value means no limit. Any value set must be a positive integer (≥1).
...
The system MUST allow admins to configure a global booking policy for the business: a minimum advance notice (in minutes) and a maximum booking horizon (in days). These define the boundaries within which customers can create bookings.
